...
void append_log(const std::string& cmd, const std::vector<std::string>& tokens){
    ...
    calc_state_machine::op_type op = calc_state_machine::PUT;

    //p stands for put and d stands for delete

    switch (cmd_char) {
    case 'p':   op = calc_state_machine::PUT;   break;
    case 'd':   op = calc_state_machine::DELETE;   break;
    };

    // Serialize and generate Raft log to append.

    std::string key = tokens[1];
    std::string value = (op == calc_state_machine::PUT) ? tokens[2] : "";
...
//explain how calc can be used in case the user doesnt input all the args
