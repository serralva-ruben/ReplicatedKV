...
class map_state_machine : public state_machine {
public:
    map_state_machine(bool async_snapshot = false)
        : cur_value_(0)
    ...
    ~map_state_machine() {}
    //operation types that can be used by the state machine
    enum op_type : int {
        PUT = 0x0,
        DELETE = 0x1,
    };

    //change payload struct to instead of holding a single integer, to now hold a key and value pair
    struct op_payload {
        int type_;
        std::pair<std::string, std::string> oprnd_;

        op_payload() : type_(0), oprnd_("", "") {}

        op_payload(int type, std::pair<std::string, std::string> oprnd)
            : type_(type), oprnd_(oprnd) {}
    };
    ...
    //change commit to now perform put and delete operations
    ptr<buffer> commit(const ulong log_idx, buffer& data) {
        op_payload payload;
        dec_log(data, payload);

        std::unordered_map<std::string, std::string> prev_value = cur_value_;
        switch (payload.type_) {
        case PUT:   
            cur_value_[payload.oprnd_.first] = payload.oprnd_.second;   
            break;
        case DELETE:   
            cur_value_.erase(payload.oprnd_.first);   
            break;
        }
                             ...
                             bool& is_last_obj)
    {
        ...
        if (obj_id == 0) {
            ...
            //need to serialize the unordered map instead of a uint64
            std::string map_str;
            for (const auto& kv : ctx->value_) {
                map_str += kv.first + ":" + kv.second + ",";
            }

            data_out = buffer::alloc(map_str.size());
            buffer_serializer bs(data_out);
            is_last_obj = true;
        }
        return 0;
                              ...
                              bool is_last_obj)
    {
        if (obj_id == 0) {
            ...
            std::string map_str = bs.get_str();  // Get the serialized map as a string.
            std::unordered_map<std::string, std::string> local_value;
    ...
    std::unordered_map<std::string, std::string> get_current_value() const { return cur_value_; }
...
private:
    ...
    std::unordered_map<std::string, std::string> cur_value_;
