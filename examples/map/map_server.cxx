...
map_state_machine* get_sm() {
    return static_cast<map_state_machine*>( stuff.sm_.get() );
}
                   ...
                   ptr<std::exception>& err)
{
    ...
    std::cout << "succeeded, "
              ...
              << ", return value: "
              << ret_value;
    std::unordered_map<std::string, std::string> value = get_sm()->get_current_value();
    std::cout<< ", state machine value: ";
...
void append_log(const std::string& cmd, const std::vector<std::string>& tokens){
    char cmd_char = cmd[0];
    map_state_machine::op_type op = map_state_machine::PUT;

    //p stands for put and d stands for delete

    switch (cmd_char) {
    case 'p':   op = map_state_machine::PUT;   break;
    case 'd':   op = map_state_machine::DELETE;   break;
    };

    // Serialize and generate Raft log to append.

    std::string key = tokens[1];
    std::string value = (op == map_state_machine::PUT) ? tokens[2] : "";
    ptr<buffer> new_log = map_state_machine::enc_log( {op, {key, value}} );
...
{
    std::cout
    << "To Put or Delete an entry in the map:\n"
    << "    put: e.g. 'put v k' where v is the key and k is the value\n"
    << "    delete: e.g. 'del v' where v is the key and the entry which has the key v will be deleted\n"
    << "\n"
...
bool do_cmd(const std::vector<std::string>& tokens) {
    ...
    //the conditions need to be changed to accodomodate the new commands
    //also the command p corresponds to put and d corresponds to delete
    if ( cmd[0] == 'p' || cmd[0] == 'd'){
...
//explain how Map can be used in case the user doesnt input all the args
void map_usage(int argc, char** argv) {
    std::stringstream ss;
    ss << "Usage: \n";
...
int main(int argc, char** argv) {
    if (argc < 3) map_usage(argc, argv);

    set_server_info(argc, argv);
    check_additional_flags(argc, argv);

    std::cout << "    -- Replicated Map with Raft --" << std::endl;
    ...
    init_raft( cs_new<map_state_machine>(ASYNC_SNAPSHOT_CREATION) );
    loop();
